...
try:
    ...
    from mutagen.id3 import APIC, ID3, TALB, TPE1, TRCK
    from mutagen.mp4 import MP4, MP4Cover
    from mutagen.oggopus import OggOpus
except ImportError:
    FLAC, Picture = None, None
    ID3, APIC, TALB, TPE1, TRCK = None, None, None, None, None
...
def set_folder_icon(folder_path: Path | str, image_source: Any) -> bool:
    ...
    try:
        ...
        if ico_path.exists() and ini_path.exists():
            return True
...
def save_playlist_cover_image(folder_path: Path | str, image_source: Any) -> Optional[Path]:
    """Save playlist cover as cover.png (or cover.jpg) in original quality."""
    if not image_source:
        return None
    try:
        folder = Path(folder_path)
        ...
        # Determine extension and save in original format
        if raw_bytes and (getattr(img, "format", "") == "JPEG" or str(image_source).lower().endswith((".jpg", ".jpeg"))):
            cover_file = folder / "cover.jpg"
            cover_file.write_bytes(raw_bytes)
        elif raw_bytes and (getattr(img, "format", "") == "PNG" or str(image_source).lower().endswith(".png")):
            cover_file = folder / "cover.png"
            cover_file.write_bytes(raw_bytes)
        else:
            cover_file = folder / "cover.png"
            img.save(str(cover_file), format="PNG")
...
def postprocess_audio_file(
    ...
    """Apply cover cropping (1000x1000 square) and tag fixing to a downloaded audio file."""
    path = Path(file_path)
    if not path.exists():
        return
    ...
    try:
        if ext == ".mp3":
            fix_mp3_cover(path)
            fix_mp3_tags(path, track_num=playlist_index, total_tracks=playlist_count, album=album)
        elif ext == ".flac":
            fix_flac_cover(path)
            fix_flac_tags(path, track_num=playlist_index, total_tracks=playlist_count, album=album)
        elif ext in (".opus", ".ogg"):
            fix_opus_cover(path)
            fix_opus_tags(path, track_num=playlist_index, total_tracks=playlist_count, album=album)
...
class MediaDownloader:
    ...
    def download(
        ...
        """Download media from the given URL with auto client rotation and playlist tracking."""
        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir, exist_ok=True)
        ...
        def internal_progress_hook(d: dict) -> None:
            nonlocal extracted_vid, extracted_thumb
